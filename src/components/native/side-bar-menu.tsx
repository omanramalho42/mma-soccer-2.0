...
const menu = [
    ...
    { title: "Dashboard", icon: <LayoutDashboard size={18} />, slug: "/management/players" },
...
const SideBarMenu:React.FC = () => {
  return (
    <div className='flex flex-col'>
        <Card className='flex flex-col gap-1 m-2 p-5 w-[15rem] xl:h-[90vh] h-full bg-gray-900'>
            {menu.map(({ icon, slug, title }, idx: number) => {
                return (
                    <Link key={idx} href={`${slug}`}>
                        <div className='flex text-white hover:bg-gray-800 transition-all p-2 flex-row items-center justify-start gap-2'>
                            { icon }
        ...
        <Card className='flex w-full gap-3 items-center bg-gray-900 m-2 p-5 w-[15rem]'>
           <Avatar>
